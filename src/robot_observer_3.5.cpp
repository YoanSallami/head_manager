...
struct ObserverStateMachine_ : public msm::front::state_machine_def<ObserverStateMachine_>
{
  ...
  struct LookingNextAction : public msm::front::state<> 
  {
      // every (optional) entry/exit methods get the event passed.
      template <class Event,class FSM>
      void on_entry(Event const&,FSM& ) {ROS_INFO("[robot_observer] Entering state: \"LookingNextAction\".");}
      template <class Event,class FSM>
  ...
  struct transition_table : mpl::vector<
    ...
    a_irow < LookingAction        , humanHandOnTable                           , &sm::stay_focus_action                                          >,
       row < LookingAction        , humanHandMove       , LookingNextAction    , &sm::focus_next_action                                          >,
      //  +-----------------------+---------------------+-----------------------+----------------------------+------------------------------------+
    ...
    a_irow < LookingNextAction    , humanHandOnTable                           , &sm::stay_focus_next_action                                     >,
     ...
     a_row < LookingNextAction    , humanHandStop       , LookingHead          , &sm::ack                                                        >
      //  +-----------------------+---------------------+-----------------------+---------------------------+------------------------------------+
    > {};
...
private:
  ...
  ros::Subscriber next_action_list_sub_; //!< current_action
  ...
  geometry_msgs::Point next_action_position_;
...
private:
  ...
  void NextActionListCallback(const supervisor_msgs::ActionsList::ConstPtr& msg)
  {
    try
    {
        if(!msg->actions.empty())
